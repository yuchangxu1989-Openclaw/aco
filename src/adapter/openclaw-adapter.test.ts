...
describe('OpenClawAdapter', () => {
  ...
  describe('killTask', () => {
    it('should POST to kill endpoint and return normal low-risk report when default workspace scan succeeds', async () => {
      mockFetch.mockResolvedValueOnce(mockResponse(200, { ok: true }));

      const result = await adapter.killTask('sess-abc123');

      const [url, opts] = mockFetch.mock.calls[0];
      expect(url).toBe('http://localhost:4141/api/v1/sessions/sess-abc123/kill');
      expect(opts.method).toBe('POST');
      expect(result).toEqual(expect.objectContaining({ sessionKey: 'sess-abc123' }));
    });

    it('returns kill impact report on successful scan (FR-K04 AC1-AC7)', async () => {
      const root = mkdtempSync(join(tmpdir(), 'aco-adapter-kill-'));
      const logs = join(root, 'logs');
      const boardPath = join(logs, 'subagent-task-board.json');
      try {
        ...
        writeFileSync(join(root, 'docs', 'product-requirements.md'), 'base\n');
        execFileSync('git', ['add', '.'], { cwd: root });
        execFileSync('git', ['commit', '-m', 'init'], { cwd: root });
        const startedAt = Date.now() - 1000;
        writeFileSync(boardPath, JSON.stringify({ tasks: [{ sessionKey: 'sess-abc123', taskId: 'task-1', label: 'spec', status: 'running', startedAt, updatedAt: startedAt + 1 }] }));
        writeFileSync(join(root, 'docs', 'product-requirements.md'), 'changed\n');
        const localAdapter = new OpenClawAdapter({ ...config, killImpactScan: { repoRoot: root, boardPath, outputDir: logs, auditLogPath: join(logs, 'dispatch-guard-events.jsonl') } });
        mockFetch.mockResolvedValueOnce(mockResponse(200, { ok: true }));

        const result = await localAdapter.killTask('sess-abc123');

        expect(result).toEqual(expect.objectContaining({
          sessionKey: 'sess-abc123',
          riskLevel: 'high',
          recommendedAction: 'consider_stash_first',
        }));
        expect('affectedFiles' in result ? result.affectedFiles[0].path : '').toBe('docs/product-requirements.md');
    ...
    it('should not throw on 404 (session already gone) and still scan', async () => {
      mockFetch.mockResolvedValueOnce(mockResponse(404, { error: 'not found' }));

      const result = await adapter.killTask('sess-gone');
      expect(result).toEqual(expect.objectContaining({ sessionKey: 'sess-gone' }));
    });
