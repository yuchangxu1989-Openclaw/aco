...
import {
  ...
  type KillImpactScanConfig,
  type KillImpactScanResult,
} from '../control/kill-impact-scan.js';
import type {
  HostAdapter,
  HostEvent,
  SpawnOptions,
...
export interface OpenClawAdapterConfig {
  ...
  /** Kill impact scanner config (FR-K04) */
  killImpactScan?: KillImpactScanConfig;
}
...
export class OpenClawAdapter implements HostAdapter {
  ...
  private readonly killImpactScanConfig: KillImpactScanConfig;
  ...
  constructor(config?: OpenClawAdapterConfig) {
    ...
    this.killImpactScanConfig = normalizeKillImpactScanConfig(config?.killImpactScan);
  }
  ...
  /**
   * Kill a running task session and return a deterministic kill impact report (FR-K04).
   */
  async killTask(sessionId: string): Promise<KillImpactScanResult> {
    const cfg = this.killImpactScanConfig;
    const killAt = Date.now();

    if (cfg.enabled === false) {
      const res = await this.callGatewayKill(sessionId);
      this.assertKillResponseOk(res);
      this.knownSessions.delete(sessionId);
      const disabled = buildKillImpactDisabledReport(sessionId, killAt);
      appendKillImpactAuditEvent('kill_impact_scan_disabled', disabled, cfg);
      return disabled;
    }

    let snapshot: ReturnType<typeof takeKillImpactSnapshot> | null = null;
    try {
      snapshot = takeKillImpactSnapshot(sessionId, cfg);
    } catch (error) {
      const res = await this.callGatewayKill(sessionId);
      this.assertKillResponseOk(res);
      this.knownSessions.delete(sessionId);
      const failed = buildKillImpactFailedReport(sessionId, error, Date.now());
      appendKillImpactAuditEvent('kill_impact_scan_failed', failed, cfg);
      return failed;
    }
    ...
    try {
      const report = scanKillImpactAfterKill(sessionId, snapshot, cfg, Date.now());
      persistKillImpactReport(report, cfg);
      return report;
    } catch (error) {
      const failed = buildKillImpactFailedReport(sessionId, error, Date.now());
      appendKillImpactAuditEvent('kill_impact_scan_failed', failed, cfg);
  ...
  private async callGatewayKill(sessionId: string): Promise<HttpResponse> {
  ...
  private assertKillResponseOk(res: HttpResponse): void {
