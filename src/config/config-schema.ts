...
export interface AcoFileConfig {
  ...
  /** 主会话异步纪律守卫配置 (FR-K01/K02/K03) */
  asyncDisciplineGuard?: {
    /** 全局开关，默认 true (FR-K01 AC6) */
    enabled?: boolean;
    /** 阻塞等待阈值，单位毫秒，默认 5000 (FR-K01 AC7) */
    ...
    /** 兼容旧计划命名，运行时会归一到 degradedRecoveryWindowMs */
    degradedRecoverIntervalMs?: number;
  };

  /** kill 后置影响扫描配置 (FR-K04) */
  killImpactScan?: {
    /** 全局开关，默认 true */
    enabled?: boolean;
    /** jsonl 报告写入目录 */
...
export function validateConfig(
  ...
  // Validate async discipline guard section
  if (cfg.asyncDisciplineGuard !== undefined) {
    if (typeof cfg.asyncDisciplineGuard !== 'object' || cfg.asyncDisciplineGuard === null) {
      errors.push({
        path: 'asyncDisciplineGuard',
        message: 'asyncDisciplineGuard must be an object',
        expected: 'object',
        actual: String(typeof cfg.asyncDisciplineGuard),
        severity: 'error',
      });
    } else {
      const guard = cfg.asyncDisciplineGuard as Record<string, unknown>;
      if (guard.enabled !== undefined && typeof guard.enabled !== 'boolean') {
        errors.push({ path: 'asyncDisciplineGuard.enabled', message: 'asyncDisciplineGuard.enabled must be a boolean', expected: 'boolean', actual: String(typeof guard.enabled), severity: 'error' });
      }
      validateNumber(errors, guard, 'asyncDisciplineGuard.maxBlockingTimeoutMs', 1);
      validateNumber(errors, guard, 'asyncDisciplineGuard.degradedRecoveryWindowMs', 60000, 3600000);
      validateNumber(errors, guard, 'asyncDisciplineGuard.degradedRecoverIntervalMs', 60000, 3600000);
      if (guard.blockingActions !== undefined && !Array.isArray(guard.blockingActions)) {
        errors.push({ path: 'asyncDisciplineGuard.blockingActions', message: 'asyncDisciplineGuard.blockingActions must be an array', expected: 'array', actual: String(typeof guard.blockingActions), severity: 'error' });
      }
      if (Object.prototype.hasOwnProperty.call(guard, 'userExemptKeywords')) {
        errors.push({
  ...
  // Validate kill impact scan section
  if (cfg.killImpactScan !== undefined) {
    if (typeof cfg.killImpactScan !== 'object' || cfg.killImpactScan === null) {
      errors.push({ path: 'killImpactScan', message: 'killImpactScan must be an object', expected: 'object', actual: String(typeof cfg.killImpactScan), severity: 'error' });
    } else {
      const scan = cfg.killImpactScan as Record<string, unknown>;
      if (scan.enabled !== undefined && typeof scan.enabled !== 'boolean') {
        errors.push({ path: 'killImpactScan.enabled', message: 'killImpactScan.enabled must be a boolean', expected: 'boolean', actual: String(typeof scan.enabled), severity: 'error' });
      }
      validateWritableDirectoryField(errors, scan, 'killImpactScan.outputDir');
      validateExistingDirectoryField(errors, scan, 'killImpactScan.repoRoot');
      validateNonEmptyString(errors, scan, 'killImpactScan.boardPath');
      validateNonEmptyStringArray(errors, scan, 'killImpactScan.highRiskPathPrefixes');
      validateNonEmptyStringArray(errors, scan, 'killImpactScan.mediumRiskPathPrefixes');
      validatePositiveIntegerField(errors, scan, 'killImpactScan.maxFileScan');
      validatePositiveIntegerField(errors, scan, 'killImpactScan.maxBoardScanBytes');
    }
  }
...
function validateNonEmptyStringArray(
  ...
  if (!Array.isArray(val) || val.some(item => typeof item !== 'string' || item.trim().length === 0)) {
    errors.push({
      path: fullPath,
      message: `${fullPath} must be an array of non-empty strings`,
      expected: 'array of non-empty strings',
      actual: Array.isArray(val) ? 'array' : String(typeof val),
      severity: 'error',
    });
  }
}
...
function validatePositiveIntegerField(
  ...
  if (typeof val !== 'number' || !Number.isInteger(val) || val <= 0) {
    errors.push({
      path: fullPath,
      message: `${fullPath} must be a positive integer`,
...
function validateExistingDirectoryField(
  errors: ConfigValidationError[],
  obj: Record<string, unknown>,
  fullPath: string,
): void {
  ...
  if (val === undefined) return;
  if (typeof val !== 'string' || val.trim().length === 0) {
    errors.push({ path: fullPath, message: `${fullPath} must be a non-empty string`, expected: 'non-empty string', actual: String(typeof val), severity: 'error' });
    return;
  }
  if (!existsSync(val) || !statSync(val).isDirectory()) {
    errors.push({ path: fullPath, message: `${fullPath} must be an existing directory`, expected: 'existing directory', actual: val, severity: 'error' });
  }
}

function validateWritableDirectoryField(
  errors: ConfigValidationError[],
  obj: Record<string, unknown>,
  fullPath: string,
): void {
  const before = errors.length;
  validateExistingDirectoryField(errors, obj, fullPath);
  const key = fullPath.split('.').pop()!;
  const val = obj[key];
  if (errors.length !== before || typeof val !== 'string' || val.trim().length === 0) return;
  try {
    accessSync(val, constants.W_OK);
  } catch {
    errors.push({ path: fullPath, message: `${fullPath} must be writable`, expected: 'writable directory', actual: val, severity: 'error' });
  }
}
  ...
  return JSON.stringify(
    {
      ...
      killImpactScan: (() => {
        const _home = process.env.OPENCLAW_HOME ?? join(homedir(), '.openclaw');
        return {
          enabled: true,
          outputDir: join(_home, 'workspace', 'logs'),
          repoRoot: join(_home, 'workspace'),
          boardPath: join(_home, 'workspace', 'logs', 'subagent-task-board.json'),
          highRiskPathPrefixes: ['src/', 'docs/'],
          mediumRiskPathPrefixes: ['reports/', 'logs/'],
          maxFileScan: 1000,
          maxBoardScanBytes: 10485760,
        };
      })(),
      dataDir: '.aco',
    },
...
export function generateAnnotatedConfig(): string {
  const lines = [
    ...
    '  "killImpactScan": {',
    '    "enabled": true,',
    `    "outputDir": "${join(process.env.OPENCLAW_HOME ?? join(homedir(), '.openclaw'), 'workspace', 'logs')}",`,
    `    "repoRoot": "${join(process.env.OPENCLAW_HOME ?? join(homedir(), '.openclaw'), 'workspace')}",`,
    `    "boardPath": "${join(process.env.OPENCLAW_HOME ?? join(homedir(), '.openclaw'), 'workspace', 'logs', 'subagent-task-board.json')}",`,
    '    "highRiskPathPrefixes": ["src/", "docs/"],',
    '    "mediumRiskPathPrefixes": ["reports/", "logs/"],',
    '    "maxFileScan": 1000,',
    '    "maxBoardScanBytes": 10485760',
