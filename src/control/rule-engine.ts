...
function createBuiltinRules(): ControlRule[] {
  return [
    ...
    // SEVO 自我进化放行（priority 110，先于 FR-C05 的 file-isolation/warn）
    {
      ruleId: 'builtin:sevo-self-evolution',
      type: 'custom',
      priority: 110,
      description: 'SEVO 修自己是被用户鼓励的现象，放行不阻止',
      enabled: true,
      evaluate(ctx: ControlRuleContext): ControlRuleResult | null {
        if (!ctx.task) return null;
        const label = (ctx.task.label ?? '').toLowerCase();
        const taskText = `${ctx.task.prompt ?? ''} ${ctx.task.label ?? ''}`;
        if (!label.includes('sevo')) return null;
        // Reference to SEVO source code (any of: projects/sevo/, packages/sevo,
        // sevo-pipeline plugin, src/cli/cmd-, src/stages/, src/pipeline/, src/engine/).
        const sevoSourceRefs = [
          /projects\/sevo(?:\/|$)/i,
          /packages\/sevo(?:\/|$)/i,
          /sevo-pipeline/i,
          /sevo\/src\/(?:cli|stages|pipeline|engine|stage-handlers|adapter|orchestrator|gates?)\b/i,
          /sevo\/index\.js\b/i,
          /sevo\/bin\/sevo\.js\b/i,
        ];
        const referencesSevoSource = sevoSourceRefs.some(re => re.test(taskText));
        if (!referencesSevoSource) return null;
        return {
          action: 'allow',
          reason: 'SEVO 自我进化：SEVO 修复/改造自身源码是被鼓励的，放行',
          ruleId: 'builtin:sevo-self-evolution',
          metadata: { label: ctx.task.label },
        };
      },
    },
