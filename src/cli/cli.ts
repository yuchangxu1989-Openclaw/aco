...
const HELP = `ACO — Agent Controlled Orchestration v${PKG_VERSION}
...
export async function main(argv: string[]): Promise<number> {
  ...
  switch (command) {
    ...
    case 'audit':
      if (getFlag(argv, 'help')) { console.log(HELP); return 0; }
      if (positionals[1] === 'kill-impact') return await auditCommand(argv.slice(argv.indexOf('audit') + 1));
    ...
    case 'doctor':
      return await healthCommand(argv.slice(argv.indexOf('doctor') + 1));
    case 'config':
...
async function cmdAudit(argv: string[]): Promise<number> {
  ...
  const positionals = getPositionals(argv);
  // positionals[0] = 'audit', positionals[1] = taskId or special report name
  const taskId = positionals[1];
  const asyncDisciplineMode = taskId === 'async-discipline';

  const auditQuery = new AuditQuery({
    filePath: asyncDisciplineMode ? await getAsyncDisciplineAuditPath() : getAuditPath(),
    retentionDays: 30,
  });
  ...
  let entries = await auditQuery.query({
    ...
    agentId: agentFilter,
    eventType: asyncDisciplineMode ? 'dispatch.process.async_discipline' : undefined,
    since,
  });
  ...
  if (jsonOutput) {
    ...
    if (asyncDisciplineMode) {
      console.log(JSON.stringify({ summary: summarizeAsyncDiscipline(entries), entries }, null, 2));
    } else if (taskId) {
  ...
  if (entries.length === 0) {
    const lines = [asyncDisciplineMode
      ? 'No async discipline audit entries found matching the filter.'
      : taskId
      ? `No audit entries found for task '${taskId}'.`
      : 'No audit entries found matching the filter.'];
    if (asyncDisciplineMode) {
      lines.push(`Hint: Audit log path: ${auditQuery.getFilePath()}`);
    }
    console.log(lines.join('\n'));
    return 0;
  }
  ...
  if (asyncDisciplineMode) {
    const summary = summarizeAsyncDiscipline(entries);
    const lines: string[] = [];
    lines.push(`Async Discipline Audit — ${entries.length} entries`);
    lines.push(`block=${summary.block} allow=${summary.allow} exempt=${summary.exempt} bypass_disabled=${summary.bypass_disabled} bypass_degraded=${summary.bypass_degraded}`);
    lines.push('─'.repeat(80));
    for (const e of entries) {
      const time = e.timestamp.includes('T') ? e.timestamp.slice(11, 19) : e.timestamp;
      const d = e.details ?? {};
      lines.push(`  ${time}  ${padR(String(d.decision ?? '-'), 16)} ${padR(String(d.action ?? '-'), 8)} timeout=${String(d.timeoutMs ?? '-')} ${String(d.reason ?? '').slice(0, 48)}`);
    }
    console.log(lines.join('\n'));
    return 0;
  }
