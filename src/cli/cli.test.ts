...
describe('CLI', () => {
  ...
  describe('aco audit (FR-E04)', () => {
    ...
    it('should read async discipline audit entries from dispatch guard events log', async () => {
      ...
      await writeFile(dispatchLogPath, JSON.stringify(decision.auditEvent) + '\n');

      const rawDispatchLog = await readFile(dispatchLogPath, 'utf-8');
      expect(rawDispatchLog).toContain('dispatch.process.async_discipline');
      expect(rawDispatchLog).toContain('"decision":"block"');

      const code = await main(['audit', 'async-discipline', '--json']);
      expect(code).toBe(0);
      const output = consoleSpy.mock.calls[0][0] as string;
      const parsed = JSON.parse(output);
      expect(parsed.summary.block).toBe(1);
      expect(parsed.entries).toHaveLength(1);
      expect(parsed.entries[0].details.action).toBe('poll');
      expect(parsed.entries[0].details.timeoutMs).toBe(600000);
    });


    it('should resolve async discipline audit path from detected OpenClaw home and read plugin hook output', async () => {
      delete process.env.ACO_ASYNC_DISCIPLINE_AUDIT_PATH;
      delete process.env.ACO_DISABLE_ROOT_CONFIG;
      await writeFile(resolve(testDir, 'openclaw.json'), JSON.stringify({ agents: { list: [{ id: 'main' }] }, plugins: { entries: {} } }));

      const dispatchLogPath = resolve(testDir, 'workspace/logs/dispatch-guard-events.jsonl');
      const pluginPath = resolve(testDir, 'aco-async-discipline-guard-test.mjs');
      ...
      handlers.get('before_prompt_build')?.({
        messages: [{ role: 'user', content: '请查一下状态' }],
      }, {
        sessionKey: 'agent:main:feishu:direct:test',
        agentId: 'main',
      });
      const hookResult = await handlers.get('before_tool_call')?.({
        toolName: 'process',
        params: { action: 'poll', timeout: 600000 },
      }, {
        sessionKey: 'agent:main:feishu:direct:test',
        agentId: 'main',
      });
      ...
      const code = await main(['audit', 'async-discipline', '--json']);
      expect(code).toBe(0);
      const output = consoleSpy.mock.calls.at(-1)?.[0] as string;
      const parsed = JSON.parse(output);
      expect(parsed.summary.block).toBe(1);
      expect(parsed.entries).toHaveLength(1);
      expect(parsed.entries[0].details.action).toBe('poll');
  ...
  describe('help and routing', () => {
    ...
    it('should route doctor to health check', async () => {
      const code = await main(['doctor']);
      expect(code).toBe(0);
      const output = consoleSpy.mock.calls[0][0] as string;
      expect(output).toContain('ACO Health Check');
    });
