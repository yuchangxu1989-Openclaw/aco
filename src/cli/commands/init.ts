...
function normalizePluginId(id: string): string {
  const aliases: Record<string, string> = { 'dispatch-guard': 'aco-dispatch-guard', 'agent-dispatch-guard': 'aco-dispatch-guard', 'run-watchdog': 'aco-run-watchdog', 'doctor-guard': 'aco-doctor-guard', 'objective-fact-guard': 'aco-objective-fact-guard', 'output-humanizer-guard': 'aco-output-humanizer-guard', 'session-context-recovery': 'aco-session-context-recovery', 'browser-session-lease': 'aco-browser-session-lease', 'spec-challenge-guard': 'aco-spec-challenge-guard' };
  ...
  return [
    {
      ...
      config: {
        ...
        timeoutProvider: hasRunWatchdog ? 'aco-run-watchdog' : 'aco',
        auditOnlyWhenDelegated: true,
      },
      conflicts: hasRunWatchdog
        ? [conflict('aco-run-watchdog', 'delegate', 'aco-run-watchdog 已存在，ACO 只记录审计，不重复执行超时处置')]
        : [],
    },
    {
    ...
    },
    {
      ...
      config: {
        ...
        source: hasDispatchGuard ? 'aco-dispatch-guard' : 'aco-default',
      },
      conflicts: hasDispatchGuard
        ? [conflict('aco-dispatch-guard', 'inherit', '继承 aco-dispatch-guard 的 ACP 全局并发上限')]
        : [],
    },
    {
      ...
      conflicts: hasDispatchGuard
        ? [conflict('aco-dispatch-guard', 'compatible', 'aco-dispatch-guard 负责 prompt 约束，ACO 补充运行时告警')]
        : [],
    },
    {
      id: 'main-session-async-discipline',
      name: '主会话异步纪律守卫',
      mode: 'enforce',
      config: {
        enabled: true,
      ...
      conflicts: hasDispatchGuard
        ? [conflict('aco-dispatch-guard', 'compatible', '与 aco-dispatch-guard 共用 before_tool_call 与 dispatch-guard-events 审计流')]
        : [],
    },
    {
      id: 'circuit-breaker',
    ...
    },
    {
      ...
      conflicts: hasRunWatchdog
        ? [conflict('aco-run-watchdog', 'bridge', '检测到 aco-run-watchdog 看板，启用 JSON 桥接，迁移期双写')]
        : [],
    },
    {
      id: 'audit-trail',
    ...
    },
    {
      ...
      config: {
        ...
        tierSource: hasDispatchGuard ? 'aco-dispatch-guard' : 'aco-inference',
      },
      conflicts: [
