...
ACO npm 包（`@self-evolving-harness/aco`）按职责域划分为以下模块：
...
```
@self-evolving-harness/aco (npm package)
├── dist/                      # 编译产物（ESM）
│   ├── core/
│   ├── models/
│   ├── adapters/
...
```json
{
  "name": "@self-evolving-harness/aco",
  "type": "module",
  "exports": {
    ".": "./dist/index.js",
...
│       ├── index.js           # Gateway Plugin 入口（import @self-evolving-harness/aco/plugin）
│       └── package.json
├── workspace/
...
└── node_modules/
    └── @self-evolving-harness/aco/  # npm 包安装位置
```
...
import { createScheduler, OpenClawAdapter } from '@self-evolving-harness/aco';
import { loadConfig } from '@self-evolving-harness/aco/plugin';
...
```typescript
import { createScheduler, TaskBoard, RuleEngine } from '@self-evolving-harness/aco';
...
Phase 1: 并行运行
─────────────────────────────────────────────────
现有插件（aco-dispatch-guard + aco-run-watchdog）继续运行
...
ACO 接管规则引擎（替代 aco-dispatch-guard 的规则逻辑）
aco-run-watchdog 的健康监测逻辑迁入 ACO HealthMonitor
现有插件降级为 thin wrapper，转发事件给 ACO

Phase 3: 完全替代
─────────────────────────────────────────────────
移除 aco-dispatch-guard 插件，ACO gateway-bridge 完全接管
移除 aco-run-watchdog 插件，ACO HealthMonitor 完全接管
...
### ADR-004: Migration Strategy from aco-dispatch-guard / aco-run-watchdog

**Status**: Decided

**Context**:
Two existing plugins handle partial orchestration duties:
- aco-dispatch-guard: admission rules, role matching, concurrency control, audit logging
- aco-run-watchdog: task monitoring, stale detection, TaskBoard management, completion handling
...
|----------|-----------|-----------|
| aco-dispatch-guard: AGENT_TIER | core/tier-router.ts | Config migrates to aco.config.json |
| aco-dispatch-guard: ROLE_TASK_MAP | rules/builtin/role-match.ts | Rules codified |
| aco-dispatch-guard: MAX_CONCURRENT_ACP | rules/builtin/concurrency-limit.ts | Rules codified |
| aco-dispatch-guard: audit log | core/audit-logger.ts | Format compatible, path configurable |
| aco-run-watchdog: STALE_MS | core/health-monitor.ts | Config migration |
| aco-run-watchdog: IDLE_ALERT_MS | core/health-monitor.ts | Config migration |
| aco-run-watchdog: upsertBoardTask | core/task-board.ts | Interface compatible |
| aco-run-watchdog: memoryBreaker | core/health-monitor.ts | Logic migration |
| aco-run-watchdog: maybeRecover | core/health-monitor.ts | Logic migration |
