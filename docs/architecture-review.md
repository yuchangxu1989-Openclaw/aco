...
**功能映射完整**：
- aco-dispatch-guard 的 AGENT_TIER → tier-router.ts
- aco-dispatch-guard 的 ROLE_TASK_MAP → role-match.ts
- aco-dispatch-guard 的 MAX_CONCURRENT_ACP → concurrency-limit.ts
- aco-run-watchdog 的 STALE_MS/IDLE_ALERT_MS → health-monitor.ts
