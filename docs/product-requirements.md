...
- 路由：检测到任务应走 SEVO 流水线 → 自动将任务路由到正确的流水线入口
...
- AC17:主会话 spawn 子任务时,若 LLM 语义判定任务会对 SEVO 受管项目产生文件变更,包括代码、spec、配置、测试、架构文档、用户文档或发布产物变更,task prompt 必须以 `sevo:` 前缀触发流水线,合法入口包括 `sevo:create`、`sevo:fix`、`sevo:implement`、`sevo:from`。缺少前缀时,dispatch-guard 采用 guide 模式向主会话注入提醒"此任务应通过 SEVO 流水线派发",写入审计事件,但不阻断 spawn。Why:裸 spawn 会绕过流水线,导致代码不可追溯、审计断裂、质量门禁形同虚设;2026-06-07 已出现连续 4 次裸 spawn 绕过流水线的真实 badcase。本 AC 与 AC12 保持边界一致:ACO 只做准入校验、提醒和审计,不改写 SEVO 的阶段定义、阶段顺序或质量门禁。验收验证:审计时构造三个样本并记录结构化结果 `{ acId, status, evidence, reason }`——(1)研发文件变更任务且 label/prompt 以 `sevo:fix` 开头时允许派发且审计事件 finalDecision=`pipeline_prefix_ok` 或等价字段;(2)研发文件变更任务缺少 `sevo:` 前缀时仍允许派发,但主会话 prompt 中出现"此任务应通过 SEVO 流水线派发"提醒,审计事件记录 finalDecision=`pipeline_prefix_guided`,requiresPipeline=true,mode=`guide`;(3)只读查询、只读调研或不产生项目文件变更的任务不触发该提醒。任一样本缺少可观测 prompt 片段、审计事件或边界样本即判定为 `fail`。
...
- AC7:每轮注入的"角色与梯队路由引导"必须包含一条明确的 SEVO 流水线前缀检查项,不得只写"任务走流水线时按提示执行"这类泛化句。检查项至少说明:凡是会对 SEVO 受管项目产生文件变更的 spawn 任务,主会话在派发前先确认 task prompt 是否以 `sevo:` 前缀开头;缺少前缀时先改为 `sevo:create`、`sevo:fix`、`sevo:implement` 或 `sevo:from` 入口,再按角色路由选择 Agent;ACO 只注入引导与审计,不替 SEVO 定义阶段。验收验证:审计时读取 `before_prompt_build` 注入后的主会话 prompt,必须能看到 `sevo:` 前缀检查项、文件变更触发条件、四个合法入口和 ACO/SEVO 边界说明;若仍只出现"任务走流水线时按提示执行"而没有可执行检查项,判定为 `fail`。
