...
- AC6:任务类型分类策略按优先级降序:声明式标注(调用方显式指定 taskType)> LLM 语义分类(自动推断)> 默认 fallback。LLM 语义分类必须调用 OpenClaw `models.providers` 中的 `penguin-classifier` provider,不得复用主会话默认模型或按历史可用性猜测模型。单次分类延迟 <= 5 秒;分类失败时按 FR-B07 的失败边界处理。验收验证:审计时构造一次未显式指定 taskType 的派发,审计事件必须记录 `providerId="penguin-classifier"`、模型 ID、latencyMs 和 semanticTaskType;若记录为 default model、缺少 providerId、或无法证明调用了 penguin-classifier,判定为 `fail`。
...
##### FR-B07:dispatch-guard 语义分类器

dispatch-guard 的分类、路由和准入判断统一由 LLM 语义分类器完成。分类器必须使用 OpenClaw 配置中的 `penguin-classifier` provider,只把正则用于确定性结构化提取,不得把关键词、正则、固定词表或 FTS 当作语义理解。Why：dispatch-guard 处理的是自然语言任务意图，用户换一种说法也应得到同一决策；用正则冒充语义理解会误拦研发任务、漏掉授权和流水线路由，最终让调度规则失真。
...
- AC3:dispatch-guard 的 LLM 分类请求必须从 OpenClaw 配置读取 `models.providers.penguin-classifier`,并在审计事件中记录 providerId、modelId、timeoutMs、latencyMs、promptVersion 和 requestId 或等价调用标识。验收验证：构造一次任务分类、一条读写区分、一条 SEVO 路由判定,三条审计事件都必须显示 `providerId="penguin-classifier"`;若使用主会话默认模型、硬编码未注册 provider、或审计证据缺失,判定为 `fail`。
- AC4:语义分类器必须覆盖以下判定路径:任务意图分类、只读与写入区分、只读调研产出检测、用户授权意图、发布任务判断、README 更新判断、批量任务判断、健康扫描判断、SEVO 流水线路由判断。验收验证:分别构造 9 类样本,审计事件必须包含对应 semantic 字段,包括 `semanticTaskType`、`readWriteIntent`、`researchOutputOnly`、`userAuthorizationIntent`、`publishIntent`、`readmeUpdateNeeded`、`batchIntent`、`healthScanIntent`、`requiresSevoPipeline`;任一判定路径无 LLM 调用证据或字段缺失即判定为 `fail`。
- AC5:读写区分必须由 LLM 判断用户是否要求改变项目产物或运行状态。正则可以提取显式文件路径、命令字符串和任务 ID,但不得仅凭“改、写、生成、发布、README、健康扫描”等词面得出写入、发布或扫描结论。验收验证:构造“分析 README 是否需要更新但只写 reports/报告”和“更新 README 面向用户说明”两个样本,前者 `readWriteIntent=read_only` 且 `researchOutputOnly=true`,后者 `readWriteIntent=write_project_doc`;若两者仅由关键词分支决定或结论相同,判定为 `fail`。
- AC6:只读调研产出检测必须同时判断任务语义和计划写入位置。LLM 判定为 research / audit / analysis 且写入仅限 reports 产物时,dispatch-guard 不触发研发 spec 准入;同一任务一旦会改代码、spec、架构、UX、测试、发布脚本、运行配置或用户文档,必须回到对应研发准入路径。验收验证:构造只写 reports 的调研任务和同时改项目文件的混合任务,审计事件分别记录 `researchOutputOnly=true, finalDecision=allow` 与 `researchOutputOnly=false, requiresSpecGate=true`。
- AC7:用户授权意图判定必须由 LLM 完成,覆盖主会话同步等待豁免、Gateway 重启授权、外部发送授权、不可逆动作授权等需要用户明确同意的场景。LLM 不可用、超时、HTTP 失败或返回不可解析结果时,授权结论必须按未授权处理,不得用关键词或正则降级放行。验收验证:mock 分类器失败后提交一条需要授权的动作,审计事件必须记录 `userAuthorizationIntent=false` 或 verdict=`error`,finalDecision 不得为授权放行。
- AC8:发布、README、批量任务和健康扫描判断必须由 LLM 输出独立布尔字段及理由。发布判断回答是否进入发布前陌生环境验证门禁;README 判断回答是否需要用户文档更新;批量判断回答是否需要 MECE 拆分;健康扫描判断回答是否属于系统健康/一致性扫描。验收验证:四类样本的审计事件必须分别出现 `publishIntent`、`readmeUpdateNeeded`、`batchIntent`、`healthScanIntent` 与自然语言 reason;若仅出现正则命中词或命中 pattern,判定为 `fail`。
- AC9:SEVO 流水线路由判断必须由 LLM 判断任务是否会对 SEVO 受管项目产生文件变更,包括代码、spec、配置、测试、架构文档、用户文档或发布产物。只读查询、只读调研和只写 reports 的任务不得触发流水线;混合输入必须拆分,研发变更部分进入 SEVO,只读部分按原任务继续。验收验证:构造研发变更、只读查询、只写 reports 调研、混合输入四类样本,审计事件必须分别记录 requiresSevoPipeline=true / false / false / splitRequired=true。
- AC10:LLM 分类失败时按判定风险分级处理。建议性治理路径（角色建议、README 提醒、批量拆分提示、健康扫描提示、只读调研放行判断）fail open:放行或不注入,并记录 classification_failed。安全和授权路径（用户授权、doctor 自动修复、Gateway 重启、不可逆动作）fail closed:按未授权或不可执行处理。流水线路由不确定时选择完整 SEVO 流水线或注入 fallback 引导,不得用正则语义兜底。验收验证:分别 mock timeout、HTTP 500、malformed JSON、provider missing 四类失败,审计事件必须记录 failureMode、riskClass、fallbackDecision,且不得出现 keywordFallback=true 或 regexFallback=true。
- AC11:正则边界必须可观测。允许的正则用途仅限确定性结构化提取:文件路径、shell 命令字符串格式、任务 ID、label 前缀、JSON 字段、URL 和配置键名;禁止用途包括任务类型、读写意图、用户授权、研发活动、发布意图、README 判断、批量判断、健康扫描判断和 SEVO 流水线路由。验收验证:代码审计或运行时 tracing 必须能列出 regexUsage 事件,每条包含 `usageType="structured_extract"` 与 extractedKind;若出现 `usageType="semantic_decision"` 或语义字段由 regex 分支写入,判定为 `fail`。
- AC12:分类结果写入 Audit Event(已有 `dispatch-guard-events.jsonl` 机制),包含原始 prompt 摘要、providerId、modelId、promptVersion、分类结果、置信度(若模型提供)、判定路径、fallbackDecision、命中的角色匹配规则和正则结构化提取证据。 验收验证：审计时按本条描述执行或复现对应操作，记录结构化结果 `{ acId, status, evidence, reason }`；`status` 必须为 `pass`，缺少 provider、语义字段、失败边界或 regexUsage 证据均判定为 `fail`。
- AC13:支持分类结果缓存--相同 prompt pattern 的任务在可配置时间窗口内(默认 5 分钟)复用上次分类结果,减少 LLM 调用开销;缓存命中不得跨越不同 providerId、promptVersion、ROLE_TASK_MAP 版本或用户授权场景。验收验证:构造同 prompt 两次分类,第二次可 `cacheHit=true`;切换 promptVersion、角色映射或授权判定场景后必须重新调用 LLM。

边界说明:本 FR 不要求用 LLM 解析确定性的命令格式、路径格式和任务 ID 格式;这些是结构化提取。凡是需要理解用户意图、任务语义、授权含义、是否产生项目变更或是否进入流水线的判断,都必须走 `penguin-classifier` 的 LLM 语义分类。
...
##### FR-H05:生产源码通用化与硬编码禁令

ACO 生产源码必须面向任意 OpenClaw 用户可移植运行，不得把某一台机器、某一个用户、某一组 agent、某一个模型或某一个默认资源池写死到运行逻辑中。所有 Agent、角色、资源池、配置路径、通知目标、模型与运行时假设都必须来自当前 OpenClaw 配置、ACO 配置或用户显式输入。Why：ACO 是对外分发的调度产品，不是某台机器上的内部脚本；一旦生产源码硬编码本地路径、agent ID、默认审计池或模型假设，第三方用户安装后会出现错派、找不到配置、审计池失真或直接启动失败。

- AC1:生产源码不得硬编码用户特定的 agent ID、本地绝对路径、role 名称、pools、IP 地址、OpenClaw 安装目录、workspace 路径、模型名称或 provider 假设。允许出现在测试 fixture、文档诊断样例、错误示例或注释说明中的内容，必须明确不参与运行时决策。验收验证：审计时扫描生产源码并输出结构化结果 `{ acId, status, evidence, reason }`；`evidence` 必须包含扫描范围、命中列表和每条命中的分类，任何运行时路径中出现上述硬编码且无法证明来自配置或用户输入，判定为 `fail`。
- AC2:Agent 池必须从 OpenClaw 当前配置的 `agents.list` 动态加载，并以该配置为唯一真相源生成可调度 agent 列表、runtime type、角色标签、梯队信息和并发边界；配置文件不存在、字段缺失、为空数组或结构非法时，系统必须进入可解释的降级状态，输出明确诊断，不得回退到内置 agent ID 列表。验收验证：分别构造正常配置、缺少 `agents.list`、空列表、非法结构四类输入，审计结果必须记录加载来源、agent 数量、降级行为和用户可读错误；出现内置 agent ID fallback 判定为 `fail`。
- AC3:默认审计池不得写死为某组 agent。未配置审计池时，系统按当前资源池的 role/tags 动态派生可审计 agent；无法派生时保持审计任务 queued 或进入 warn/diagnostic 状态并说明原因；用户显式配置审计池时，以配置派生结果为准，并校验其中每个 agent 都存在于当前 `agents.list`。验收验证：构造空审计池、可由 role 派生、由配置派生、配置引用不存在 agent 四类场景，审计结果必须分别证明系统未使用硬编码默认池、派生逻辑可观测、缺失配置有清晰行为。
- AC4:OpenClaw 配置路径解析必须遵循明确优先级链：`OPENCLAW_CONFIG_PATH` 指向的文件优先；其次使用 `OPENCLAW_HOME` 下的配置；再尝试当前工作目录或项目根可发现的配置；最后尝试用户 home 目录下的默认配置。每一步失败都必须记录原因，成功后记录最终来源；不得硬编码 `/root/.openclaw/openclaw.json` 或任何个人目录作为运行时唯一入口。验收验证：分别设置或移除上述四类位置的配置，审计结果必须显示命中的优先级、最终路径来源和失败链路；若源码只读固定绝对路径或优先级不符合本 AC，判定为 `fail`。
- AC5:模型、provider、runtime 类型和 role 名称不得根据历史命名习惯、agentId 前缀或本机残留目录推断。系统只能使用当前配置中的显式字段，或在 spec 定义的降级规则下给出 warn/diagnostic；推断结果必须可追溯到配置字段或用户输入。验收验证：构造 agentId 与 role/tier 不一致、残留目录存在但配置不存在、provider 缺失三类场景，审计结果必须证明系统以配置为准且不按名称猜测。
- AC6:每次涉及生产源码通用化、配置发现、agent 池加载、默认池派生或分发前验收的交付，必须包含生产源码硬编码扫描审计报告。报告至少列出扫描命令或扫描器、扫描范围、排除规则、命中项、误报说明、结论和修复状态；没有报告不得视为交付完成。验收验证：审计交付物中必须存在硬编码扫描报告，且报告结论覆盖本 FR 的禁止项；报告缺失、只口头说明、未列扫描范围或未区分生产源码与测试/文档，判定为 `fail`。

边界说明：本 FR 约束生产源码和可复制示例，不禁止测试用例构造假 agent、假路径或假配置，也不禁止文档中展示已脱敏的诊断样例。测试、文档和诊断样例中的固定值必须与运行时决策隔离，不能被生产逻辑读取为默认行为。
...
##### FR-K04a:kill 后过程产出污染处置门禁

**Why**

kill 掉运行中的任务不等于任务影响已经结束。被 kill 的 Agent 可能已经把半成品脚本、未完成文档、临时配置或错误 diff 写到磁盘上。若主会话没有先检查这些过程产出,就把新任务继续派到同一文件域,后续 Agent 很可能会读到脏文件,或者在半成品基础上继续开发,把一次可控的 kill 变成连续污染。

FR-K04 已经解决"kill 后知道影响了什么"的问题,但它只负责扫描和报告。ACO 还需要在调度层补上后一半闭环:同一文件域在污染未判定、未清理、未明确保留前,不得继续派发新任务。这个责任属于 ACO,因为它发生在 kill 与下一次派发之间,目标是保护调度队列和工作区状态,不是定义研发流水线内容。

**FR 描述**

当运行中任务被 kill 后,ACO 必须把该任务命中的文件域标记为待处置状态,并基于 kill 影响扫描结果检查磁盘上是否存在过程产出污染。检查范围至少覆盖工作树 diff、新增未跟踪文件、任务运行期间 mtime 变化的文件、以及看板中记录的任务产出路径。只有在污染已清理、已回滚、已暂存隔离,或被主会话明确标记为可保留后,ACO 才允许把新的任务派发到同一文件域。

本 FR 不要求 ACO 自动删除、自动回滚或自动 stash。ACO 的职责是强制检查、记录处置结论、阻断同文件域后续派发;清理动作可以由主会话、用户或被派发的专门清理任务完成。跨文件域、不读取也不修改该文件域的任务不受本 FR 阻断。

**AC**

- AC1(待处置状态):任何 running 任务被 kill 后,ACO 必须根据该任务 prompt、看板元数据、kill 影响报告和已知产出路径识别文件域,并为命中的文件域创建 `pending_cleanup` 状态记录。记录至少包含被 kill 的 taskId/sessionKey、killAt、文件域、affectedFiles、riskLevel、recommendedAction 和状态来源。验收验证:kill 一个已写入 `docs/` 或 `scripts/` 文件的运行中任务后,看板或审计记录中能看到对应文件域进入 `pending_cleanup`;缺少 taskId/sessionKey、文件域或 affectedFiles 均判定为 fail。
- AC2(什么算检查过):同一文件域从 `pending_cleanup` 进入可派发状态前,必须完成一次可追溯检查。检查证据至少包含 kill 影响报告、当前 `git status`/工作树 diff 结果、新增未跟踪文件列表、mtime 落在任务运行窗口内的文件列表,以及检查人或检查任务的结论。只看 completion 文本、只凭 Agent 自报、只凭记忆或只看任务状态,都不算检查过。验收验证:审计记录必须能还原"检查了哪些文件、发现了什么、结论是什么";证据缺任一类即 fail。
- AC3(什么算清理完):满足以下任一条件才算清理完:相关过程产出已回滚到 kill 前可信版本;相关半成品文件已删除或移入隔离位置且不会被后续任务默认读取;相关 diff 已被主会话明确确认可保留并记录理由;相关改动已整理成可审计的独立提交或暂存隔离包。清理结论必须写入状态记录,并把文件域状态从 `pending_cleanup` 改为 `cleaned` 或 `accepted`. 验收验证:完成清理后再次读取状态记录和工作树,能看到处置方式、处置时间、处置证据;仍存在未解释的同文件域脏 diff 或未跟踪文件时判定为 fail。
- AC4(什么时候必须回滚或隔离):当 affectedFiles 命中产品 spec、架构文档、源码、配置、脚本、测试或发布产物,且无法证明这些内容完整、正确、可继续作为后续任务输入时,必须回滚到可信版本或隔离这些过程产出。报告类、日志类文件若只作为只读证据且不会被后续任务当作源输入,可以记录后保留。验收验证:构造一个被 kill 任务留下半成品 `scripts/` 或 `docs/` 文件的场景,若系统允许同域新任务在未回滚、未隔离、未确认保留前派发,判定为 fail。
- AC5(同文件域派发阻断):当存在 `pending_cleanup` 文件域时,ACO 必须阻断任何会读取、修改或基于该文件域继续工作的后续任务派发,阻断原因必须包含被 kill 的 taskId/sessionKey、待处置文件域、affectedFiles 摘要和需要完成的处置动作。不同文件域且无依赖关系的任务不得被误阻断。验收验证:kill 后立即向同一文件域派发新任务应被阻断;向无交集文件域派发任务应正常通过。
- AC6(解除阻断):只有当同文件域状态为 `cleaned` 或 `accepted`,且审计记录包含 AC2/AC3 要求的检查与处置证据时,ACO 才能解除派发阻断。解除记录必须保留原 kill taskId/sessionKey 与处置结论的关联。验收验证:清理前派发被阻断,清理记录补齐后同一任务可派发;若缺少处置证据但手工改状态为可派发,判定为 fail。
- AC7(降级路径):若 kill 影响扫描失败或无法确定文件域,ACO 必须按保守策略处理:将任务所在项目根或最近可识别目录标记为 `pending_cleanup_unknown_scope`,阻断可能重用该范围的派发,并提示主会话先做人工检查。不得因为扫描失败就默认安全放行。验收验证:mock kill 影响扫描失败后,同项目后续写任务不能直接派发;审计记录包含 scanFailed 和 unknown scope 说明。

**与现有能力的关系**

- 与 FR-K04:FR-K04 负责 kill 前后扫描并输出影响报告;本 FR 使用该报告作为派发门禁输入,补齐"扫描之后必须处置污染"的闭环。FR-K04 不自动做决策,本 FR 也不自动删除文件,但会在污染未处置前阻断同文件域派发。
- 与 FR-K20 / FR-K20a:FR-K20 负责派发前识别任务影响域;本 FR 复用影响域概念判断"后续任务是否命中待清理文件域"。FR-K20 关注任务范围是否过宽,本 FR 关注已知污染范围是否被复用。
- 与 SEVO:本 FR 不定义需求、架构、编码或审计阶段内容,只治理 kill 后工作区状态与下一次派发之间的安全边界。研发内容是否正确仍由 SEVO 流水线和对应审计门禁负责。
...
- AC7:插件只分析 spawn-time task prompt,不得扫描已组装的主会话 prompt、其他 L2 插件注入文本或历史消息。task prompt 为空时静默通过,不触发 LLM 分析、不注入提醒。Why:如果把系统提示、历史消息或其他插件注入内容当成任务范围,会把规则文本里的路径误判成当前任务影响域,导致无关拆分建议污染派发决策。验收验证:分别构造带有 `taskPrompt` 的 spawn 事件、只有 assembled prompt 的 `before_prompt_build` 事件、task prompt 为空的事件;只有第一类允许进入影响域分析,后两类无注入且无 split_suggestion 审计事件。
- AC8:开发任务识别和隐式文件域判断必须由 LLM 语义分析完成,不得用关键词、正则或固定项目名清单替代。LLM 配置缺失、超时、HTTP 失败、返回不可解析 JSON 或分类失败时,插件必须 fail open:静默通过、不注入、不阻断,且不得启用正则语义 fallback。Why:影响域预检是建议性治理,错误阻断或误提醒比漏提醒更伤害调度;语义判断用关键词兜底会重演误判和系统瘫痪问题。验收验证:mock LLM config missing、timeout、HTTP 500、malformed JSON 四类失败,插件返回 null,审计日志中不得出现 split_suggestion_injected;mock 非开发任务时也返回 null。
- AC9:插件可以用结构化路径提取规则读取 task prompt 中明确写出的文件路径,但该规则只能提取路径 token 和归一化路径格式,不得据此判断任务是否为开发任务、不得识别项目语义、不得替代 LLM 的开发任务分类。Why:路径格式提取是确定性结构解析,能提高显式文件域覆盖率;把路径正则升级成语义判断会违反 ACO 插件正则禁令。验收验证:构造含显式路径的开发任务,LLM 判定为开发后结构化路径被合并到 domains;构造同样路径但 LLM 判定为非开发或 LLM 失败时,插件不得仅凭路径注入提醒。
- AC10:文件域归一化必须遵循可移植的目录结构规则:以 `projects/<name>`、`extensions/<name>`、`projects/<name>/src`、`src/<module>` 等顶层结构归并,不得硬编码当前机器的项目名、agentId 或个人路径。绝对 workspace 路径只允许被归一化为相对域。Why:ACO 面向第三方环境,影响域规则必须随任意 OpenClaw workspace 工作,不能只识别当前宿主机器上的项目。验收验证:输入 `/root/.openclaw/workspace/projects/demo/src/a.ts`、`projects/demo/src/b.ts`、`extensions/foo/index.js`、`src/bar/x.ts`,输出域分别归并为 `projects/demo/src`、`extensions/foo`、`src/bar`,且配置或源码中不存在项目名白名单。
- AC11:当域数量超过阈值时,注入文本必须明确这是“仅供参考不阻断”的影响域预检,包含域数量、阈值、每个建议子任务的聚焦域和预期文件列表;最多展示前 5 个域,避免长 prompt 被拆分建议淹没。Why:提醒要帮助主会话拆小任务,不能变成新的硬拦截或长文本噪音。验收验证:构造 4 个域和 7 个域两个样本;4 域样本注入 4 条子任务建议,7 域样本最多注入 5 条,文本包含“不阻断/仅供参考”等等价语义、域数量和阈值。
- AC12:触发拆分建议时必须写入结构化审计事件,字段至少包含 pluginId、eventType、ruleId、decision、reason、label、domainCount、threshold、domains、splitSummary、classifier.source、classifier.model、classifier.isDevelopment、sessionKey。decision 取值使用建议性语义,不得记录为 block。Why:影响域提醒需要可追溯,审计者要能复盘为什么提醒、LLM 如何分类、建议拆成哪些域,同时确认它没有阻断派发。验收验证:触发一次 >3 域样本后读取审计 JSONL,上述字段完整存在,`domainCount > threshold`, `decision` 不为 `block`, `classifier.source=llm`。
