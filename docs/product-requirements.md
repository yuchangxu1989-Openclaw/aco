...
**主人群（P0）**：一个人带着多个 Agent 做产品的独立开发者 / Solo Founder。特征：同时调度 3-10 个 Agent，需要规则不被长上下文稀释、失败自动接管、并行调度最大化、运行状态对人透明。下列场景围绕这一主人群展开，小团队等其他形态为次要延伸。
...
ACO 是面向单人或小团队维护多 Agent 基础设施场景的 Agent Runtime Governance（调度治理中心）。它通过派发前治理、运行时看护、kill-impact-scan、failure attribution、tier upgrade decisions、circuit breaker、任务看板、审计日志和通知闭环，解决多 Agent 协作中错派、自审、卡死、失败原样重派、kill 后误清理和状态误报这些关键断点，让 Agent 系统获得可配置、可追溯、可恢复、可升级的运行时治理能力。ACO 常被误解为通用任务执行器、项目管理看板或完整研发流水线；它的核心价值是把“怎么拆、怎么派、怎么看护、失败后怎么升级、危险动作前怎么看影响”固化成可执行治理规则。

#### 6.1.1 需求范围

本产品负责 Agent 任务从创建、派发、运行时看护到终态闭环的调度治理，包括资源池与角色映射、派发准入校验、并发控制、超时与卡死处理、失败归因、梯队升级、熔断、kill 影响扫描、看板投影、审计记录和通知触发。

本产品可以与 SEVO、OpenClaw Gateway、宿主 IM 渠道、开发 Agent、审计 Agent、UX Agent 和发布验证链路协同，但不负责定义研发阶段、撰写需求规格、生成架构方案、执行代码实现、执行审计结论或替代宿主完成最终用户交互。

当任务进入需求冻结、架构设计、代码实现、独立审计、UX 验证、发布执行或业务内容生产阶段时，应交由对应产品、流水线阶段或专业 Agent 继续处理；ACO 保留调度准入、运行状态、失败处理和风险影响报告职责。

非范围声明：ACO 不负责产品需求决策、架构方案产出、代码实现、审计结论、视觉设计、内容创作、发布平台能力和 IM 渠道本身，这些分别交给 SEVO、架构/开发/审计/UX Agent、宿主发布系统和宿主通知适配器。ACO 的边界是：管理 Agent 任务运行时治理闭环。本产品只负责自己所在环节的闭环，同时向其他模块输出派发决策、任务状态、失败归因、升级建议、熔断状态、kill 影响报告、审计事件和通知触发结果。
...
- **Guard**：派发前、运行中或回复前的治理规则执行器，用来拦截错派、自审、超时、事实误报、异步阻塞、高风险命令等问题。
...
- AC3:无 role 声明时的渐进式降级--单 Agent 模式:跳过角色匹配,所有任务允许派发;多 Agent 无 role 声明:warn 模式,记录日志但不拦截;有 role 声明:enforce 模式,拦截角色不匹配的派发。 验收验证：审计时按本条描述执行或复现对应操作，记录结构化结果 `{ acId, status, evidence, reason }`；`status` 必须为 `pass`，`evidence` 必须包含可观测输出（文件路径、CLI 输出、API 响应、页面截图、审计事件或状态字段之一），缺少证据、字段值不符或无法复现均判定为 `fail`。
...
- AC3:分类失败(LLM 超时、不可用、返回无法解析的结果)时 fallback 到 warn 模式--放行派发但记录告警日志,不硬拦截。 验收验证：审计时按本条描述执行或复现对应操作，记录结构化结果 `{ acId, status, evidence, reason }`；`status` 必须为 `pass`，`evidence` 必须包含可观测输出（文件路径、CLI 输出、API 响应、页面截图、审计事件或状态字段之一），缺少证据、字段值不符或无法复现均判定为 `fail`。
...
- AC6:任务状态发生变化时,系统向已配置的用户可见 IM 渠道推送任务看板卡片,触发范围包含任务创建入队、派发运行、完成、失败、超时和取消。卡片展示当前看板快照,至少包含 running、queued、recent completed 三类任务摘要。 验收验证：审计时按本条描述执行或复现对应操作，记录结构化结果 `{ acId, status, evidence, reason }`；`status` 必须为 `pass`，`evidence` 必须包含可观测输出（文件路径、CLI 输出、API 响应、页面截图、审计事件或状态字段之一），缺少证据、字段值不符或无法复现均判定为 `fail`。
...
- AC11:L2 在主会话回复前检测是否有未发送的 `pendingClosure`;如果有且主会话未调用 `lark-cli`,则注入强制指令阻止回复(拦截模式)。 验收验证：审计时按本条描述执行或复现对应操作，记录结构化结果 `{ acId, status, evidence, reason }`；`status` 必须为 `pass`，`evidence` 必须包含可观测输出（文件路径、CLI 输出、API 响应、页面截图、审计事件或状态字段之一），缺少证据、字段值不符或无法复现均判定为 `fail`。
...
ACO 在任务状态变化时,自动复用 OpenClaw 已配置的用户可见 IM 渠道推送任务看板卡片。用户只要完成 OpenClaw 基础 IM 渠道配置并启用 ACO,就能在派发第一个任务时收到看板卡片,不需要为 ACO 额外填写用户 ID、凭据路径或单独通知目标。

- AC1:ACO 从 OpenClaw 当前配置中自动识别已启用的 IM 渠道,支持飞书、Telegram、WhatsApp 和通用 Webhook。未配置任何可用渠道时,任务调度照常运行,通知链路静默跳过并记录可诊断日志。 验收验证：审计时按本条描述执行或复现对应操作，记录结构化结果 `{ acId, status, evidence, reason }`；`status` 必须为 `pass`，`evidence` 必须包含可观测输出（文件路径、CLI 输出、API 响应、页面截图、审计事件或状态字段之一），缺少证据、字段值不符或无法复现均判定为 `fail`。
- AC2:飞书渠道可用时,ACO 使用 OpenClaw 飞书渠道已有的应用凭据和会话归属信息确定通知目标,不得要求用户在 ACO 中重复配置 open_id、app_id、app_secret 或凭据文件路径。通知目标发现遵循以下优先级:优先使用 Gateway 运行时上下文中的当前活跃用户 IM 身份;仅在运行时上下文不可用时,才回退读取 OpenClaw 飞书渠道已有的联系人配置。只要用户通过飞书与 OpenClaw 对话过一次,就应自动成为看板卡片推送目标,不需要额外配置。 验收验证：审计时按本条描述执行或复现对应操作，记录结构化结果 `{ acId, status, evidence, reason }`；`status` 必须为 `pass`，`evidence` 必须包含可观测输出（文件路径、CLI 输出、API 响应、页面截图、审计事件或状态字段之一），缺少证据、字段值不符或无法复现均判定为 `fail`。
- AC3:新装 OpenClaw、已配置飞书渠道并启用 ACO 后,用户派发第一个任务时,应自动收到一张任务看板卡片。卡片内容与 FR-E02 的看板视图一致,展示 running、queued、recent completed 的当前快照。 验收验证：审计时按本条描述执行或复现对应操作，记录结构化结果 `{ acId, status, evidence, reason }`；`status` 必须为 `pass`，`evidence` 必须包含可观测输出（文件路径、CLI 输出、API 响应、页面截图、审计事件或状态字段之一），缺少证据、字段值不符或无法复现均判定为 `fail`。
- AC4:任务状态变化均触发看板卡片推送,范围包含 queued、running、succeeded、failed、timed_out 和 cancelled。短时间内多次状态变化可以合并为一次最新快照,但不得漏掉最终状态。 验收验证：审计时按本条描述执行或复现对应操作，记录结构化结果 `{ acId, status, evidence, reason }`；`status` 必须为 `pass`，`evidence` 必须包含可观测输出（文件路径、CLI 输出、API 响应、页面截图、审计事件或状态字段之一），缺少证据、字段值不符或无法复现均判定为 `fail`。
- AC5:看板卡片推送属于 ACO 运行时看护能力的一部分,随任务状态更新同步执行,不依赖用户额外启动脚本或手工执行命令。 验收验证：审计时按本条描述执行或复现对应操作，记录结构化结果 `{ acId, status, evidence, reason }`；`status` 必须为 `pass`，`evidence` 必须包含可观测输出（文件路径、CLI 输出、API 响应、页面截图、审计事件或状态字段之一），缺少证据、字段值不符或无法复现均判定为 `fail`。
- AC6:推送失败只影响本次通知送达,不得阻断任务派发、状态流转、失败重试或后续推进链。失败原因写入通知送达记录和运行日志,供用户排查。 验收验证：审计时按本条描述执行或复现对应操作，记录结构化结果 `{ acId, status, evidence, reason }`；`status` 必须为 `pass`，`evidence` 必须包含可观测输出（文件路径、CLI 输出、API 响应、页面截图、审计事件或状态字段之一），缺少证据、字段值不符或无法复现均判定为 `fail`。
...
- AC1：插件在 Gateway 事件循环中拦截 `message:outbound` 事件（或等价的出站消息钩子），在消息实际送达用户可见渠道之前执行检测逻辑。拦截点必须在 L2 插件层，独立于主会话上下文。 验收验证：审计时按本条描述执行或复现对应操作，记录结构化结果 `{ acId, status, evidence, reason }`；`status` 必须为 `pass`，`evidence` 必须包含可观测输出（文件路径、CLI 输出、API 响应、页面截图、审计事件或状态字段之一），缺少证据、字段值不符或无法复现均判定为 `fail`。
...
- AC2:当 timeout >= `asyncDisciplineGuard.maxBlockingTimeoutMs`(默认 5000 毫秒)时,守卫返回 block 决策,工具调用被取消,主会话收到结构化错误响应,响应内容包含:命中规则名、被拦截的工具调用参数摘要、推荐的合规路径文本。 验收验证：审计时按本条描述执行或复现对应操作，记录结构化结果 `{ acId, status, evidence, reason }`；`status` 必须为 `pass`，`evidence` 必须包含可观测输出（文件路径、CLI 输出、API 响应、页面截图、审计事件或状态字段之一），缺少证据、字段值不符或无法复现均判定为 `fail`。
...
- AC4:命中授权意图时本次工具调用放行,未命中则继续拦截;豁免状态不缓存到下一次工具调用。豁免判定的具体 LLM 语义、超时与异常处理由 FR-K02 定义。 验收验证：审计时按本条描述执行或复现对应操作，记录结构化结果 `{ acId, status, evidence, reason }`；`status` 必须为 `pass`，`evidence` 必须包含可观测输出（文件路径、CLI 输出、API 响应、页面截图、审计事件或状态字段之一），缺少证据、字段值不符或无法复现均判定为 `fail`。
- AC5:每次拦截、放行、豁免都写入 `dispatch-guard-events.jsonl`,记录字段包括 timestamp、sessionKey、agentId、toolName、action、timeoutMs、decision(block / allow / exempt / bypass_disabled / bypass_degraded)、exemptKeyword(命中时恒为 null,仅为兼容历史 schema 保留)、recentUserMessageHash(最近一条用户消息的 SHA-256 截断 16 位,用于审计追溯且不泄露内容)。 验收验证：审计时按本条描述执行或复现对应操作，记录结构化结果 `{ acId, status, evidence, reason }`；`status` 必须为 `pass`，`evidence` 必须包含可观测输出（文件路径、CLI 输出、API 响应、页面截图、审计事件或状态字段之一），缺少证据、字段值不符或无法复现均判定为 `fail`。
- AC6:`asyncDisciplineGuard.enabled` 控制全局开关,默认 true。设为 false 时所有拦截逻辑跳过,但仍写审计日志(decision = bypass_disabled),保证可观测性。 验收验证：审计时按本条描述执行或复现对应操作，记录结构化结果 `{ acId, status, evidence, reason }`；`status` 必须为 `pass`，`evidence` 必须包含可观测输出（文件路径、CLI 输出、API 响应、页面截图、审计事件或状态字段之一），缺少证据、字段值不符或无法复现均判定为 `fail`。
...
- AC9:守卫与 aco-run-watchdog 的 completion event 推送链路完全解耦。子 Agent 完成时 completion event 的派发延迟必须保持在毫秒级,不因守卫拦截了主会话的 poll 而延迟或丢失。 验收验证：审计时按本条描述执行或复现对应操作，记录结构化结果 `{ acId, status, evidence, reason }`；`status` 必须为 `pass`，`evidence` 必须包含可观测输出（文件路径、CLI 输出、API 响应、页面截图、审计事件或状态字段之一），缺少证据、字段值不符或无法复现均判定为 `fail`。
- AC10:守卫只对 agent=main 的 session 生效。子 Agent session(任何 agentId != main)的 process 工具调用不被拦截,允许子 Agent 内部使用 poll 等待自己派出的孙子任务。 验收验证：审计时按本条描述执行或复现对应操作，记录结构化结果 `{ acId, status, evidence, reason }`；`status` 必须为 `pass`，`evidence` 必须包含可观测输出（文件路径、CLI 输出、API 响应、页面截图、审计事件或状态字段之一），缺少证据、字段值不符或无法复现均判定为 `fail`。
...
- AC12:CLI 命令 `aco audit async-discipline` 输出最近 N 条(默认 50)主会话异步纪律相关的审计事件,按 decision 分组统计拦截/放行/豁免次数。 验收验证：审计时按本条描述执行或复现对应操作，记录结构化结果 `{ acId, status, evidence, reason }`；`status` 必须为 `pass`，`evidence` 必须包含可观测输出（文件路径、CLI 输出、API 响应、页面截图、审计事件或状态字段之一），缺少证据、字段值不符或无法复现均判定为 `fail`。
...
`dispatch-guard` 检测到开发任务缺少 spec 引用时,在 `sessions_spawn` 路径上确定性阻断而非仅返回 prompt 提示;FR-K06 的 prompt 注入路径升级为 spawn 拦截。
...
- AC2:demo 输出涵盖任务进入、规则拦截判定、派发决策、失败重派、最终完成五个阶段的可读日志。 验收验证：审计时按本条描述执行或复现对应操作，记录结构化结果 `{ acId, status, evidence, reason }`；`status` 必须为 `pass`，`evidence` 必须包含可观测输出（文件路径、CLI 输出、API 响应、页面截图、审计事件或状态字段之一），缺少证据、字段值不符或无法复现均判定为 `fail`。
...
ACO 输出运行数据供 SEVO Web dashboard 消费时,为每个数据项标注来源类型,区分"runtime ledger 真实数据"与"文件派生/合成视图",供 dashboard 面板呈现,避免用户把派生视图当成实时状态。
...
### FR-K15: 调度 SEVO implement 任务时注入测试门禁

**背景**：测试全绿门禁由 SEVO 在 implement 阶段定义。ACO 调度 SEVO 流水线 implement 任务时，需要在任务派发环节把该门禁要求落到 task prompt，确保实现完成后测试被执行。
...
### FR-K16: 调度发布任务时注入陌生环境验证门禁

**背景**：发布前的陌生环境验证（stranger-verify）由 SEVO 在发布流程中定义。ACO 调度发布类任务时，需要在派发环节把该验证门禁追加到任务，确保 npm publish 前在干净环境验证包可用。
...
- AC5:当系统判定任务类型与目标 Agent 角色不匹配时，必须拦截本次派发决策并向主会话注入 advisory 级警告提示；提示内容至少包含识别出的任务类型、目标 `agentId` 的当前角色、为什么不匹配、以及建议改派的角色或 Agent 列表。 验收验证：审计时按本条描述执行或复现对应操作，记录结构化结果 `{ acId, status, evidence, reason }`；`status` 必须为 `pass`，`evidence` 必须包含可观测输出（文件路径、CLI 输出、API 响应、页面截图、审计事件或状态字段之一），缺少证据、字段值不符或无法复现均判定为 `fail`。
