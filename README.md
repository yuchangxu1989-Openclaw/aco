...
## 自愈进化 / Self-Healing Governance

Agent 犯错后，用户一句纠偏不会停在聊天里。ACO 会顺着这句话追根因，判断要不要创建或优化 L2 规则，并当场派任务把修复固化到规则层。下次再遇到同类场景，系统已经变聪明了。

传统系统靠人工维护规则；ACO 的规则在每次犯错后自动进化。
...
- Node.js 18 或更高版本。
- 只想先看 ACO 的运行效果：不需要 OpenClaw、provider、网络或 API key，直接运行 `aco demo`。
- 要把 ACO 接入真实运行时：需要本机已有 OpenClaw 环境，并且 `OPENCLAW_HOME/openclaw.json` 或 `~/.openclaw/openclaw.json` 可读。
- 默认 async discipline guard 需要在 `openclaw.json` 的 `models.providers` 里配置可用模型。默认值是 `penguin-main/claude-opus-4-7`；也可以在 `aco.config.json` 里改成你已有的 provider/model，或临时关闭：
...
如果你的 OpenClaw 配置不在默认位置，先指定：

```bash
OPENCLAW_HOME=/path/to/.openclaw aco init
```

ACO 的数据目录默认与运行模式一致：检测到 OpenClaw 时使用 `<openclaw-home>/aco-data`；未检测到 OpenClaw 时使用当前目录 `.aco`。你可以在初始化时固定目录，后续 CLI 也会使用同一个目录：

```bash
aco init --data-dir ~/.openclaw/aco-data
ACO_DATA_DIR=~/.openclaw/aco-data aco health
```
